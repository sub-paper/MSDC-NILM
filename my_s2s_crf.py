...
import model
# our schme multiple states with CRF layer
params_appliance = {
    'kettle':{
        'window_len':599,
...
tra_provider = data_provider.S2S_State_Slider_drop_winlen(batch_size=args.batch_size, 
                            shuffle=True, offset=offset, length=window_len, out_len=out_len, threshold=threshold)
...
for epoch in range(args.n_epoch):
    ...
    for batch in tra_provider.feed(**tra_kwag):
        ...
        loss = F.mse_loss(o_train, y_train) + m.calc_crf_loss(os_train, s_train)
        loss.backward()
        optimizer.step()
    ...
    with torch.no_grad():
        for batch in val_provider.feed(**val_kwag):
            ...
            val_loss += F.mse_loss(o_val, y_val).item() + m.calc_crf_loss(os_val, s_val).item()
            n_batch_val += 1
...
pred, pred_p, pred_s, gt, gt_s = np.vstack(pred/ave), np.vstack(pred_p/ave_s), np.vstack(pred_s/ave_s), np.vstack(gt), np.vstack(gt_s)
...
pred_sh = np.array(m.decode(torch.from_numpy(pred_s).float().cuda().unsqueeze(0)))
